        ...
        (
           ...
           *X_[specieJ] 
           /sqrt
            (
                dimensionedScalar("one", dimless, 1.0)
              + thermo_.composition().W(specieI)/thermo_.composition().W(specieJ)  
            )
        );
    }
...
:
    laminarTransport(U, moleFraction_, thermo, mesh),
    Dmix_(n_),
    Vcorr_
    (
        IOobject
    ...
    CutOff_(mixtureAverageDict_.lookupOrDefault("CutOff", 5.0))
{
   ...
   if (thermophoresis_)
   {
       forAll(Y_, specieI)
       {
           if (thermo_.composition().W(specieI) <= this->CutOff_)
           {
               dictionary CoeffsDict =
...
) const
{
    ...
    if (thermophoresis_ && (thermo_.composition().W(i) <= this->CutOff_))
    {
        phiT_ = 
            linearInterpolate
            (
